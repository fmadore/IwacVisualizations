...
- **Bubble maps share one vocabulary** (`charts/shared/maplibre.js`): `P.mapColor` for a token, `P.bubbleLayer` / `P.bubblePaint` with `P.countRadius` (sqrt) and a sort key for a count layer, `P.attachMapClickPopup` for the popup (registered once, never inside `onStyleReady` — a theme swap would stack a copy per toggle), `P.fitToPoints` for the view, `P.buildMapPopup` for every popup body. A map whose bubbles change with a filter is `P.createFilteredPlacesMap` (`shared/places-map.js`). Do not hand-roll a paint block or a `resolvePrimary` trio; v1.63.0 deleted six of each.
- **Nothing may read `maplibregl` synchronously.** MapLibre 6 is ESM-only, so the loader `import()`s it *in parallel* with the classic script chain — the global is routinely absent when a panel renders, and a `createIwacMap` that returns null means "not yet", never "never". Wrap creation in `P.withMaplibre(host, build)` (render-once panels) or `P.deferMaplibre(host, factory, methods)` (panels that hand a live controller to a toolbar or sidebar, which cannot take a promise). `npm run lint:maplibre` fails the build otherwise; the exemption is *defining* the gate, not being on a list. This is the whole reason v1.53.0's co-occurrence network showed "Map library unavailable" in production and worked on every warm cache. **d3 is the opposite case** — its UMD builds ride the ordered `$scripts` chain, so `shared/graph-force.js` and friends may read the `d3` global directly; the guard asserts that asymmetry rather than assuming it.
...
- **JS is bundled, and the load order is data.** `asset/js/bundles.json` names six shared bundles, the shared panel sets and one bundle per block (panels in order, orchestrator last); `npm run build:js` (esbuild) writes `asset/js/dist/**` with sourcemaps, and both sources and `dist/` are committed. The build fails when a source is missing from the manifest, listed twice, or gone — add every new file to the manifest. Shared code (`charts/shared/**`) goes in a shared bundle, never in a block bundle: the loader de-duplicates by URL, so a shared file inlined into two block bundles would execute twice.
- **Templates name a bundle, not files.** A template declares `'needs' => [...]` and `'bundle' => '<name>'` through `view/common/iwac-assets.phtml` — do not write raw `headScript`/`headLink` in templates, and do not list panel files there (`lint:blocks` fails on `'panels'` / `'orchestrator'`). `blockCss` accepts a list when a block layers its own sheet over a shared base.
- **`npm run lint:js` (ESLint `recommended`, browser scripts against the `IWACVis` / `echarts` / `maplibregl` / `d3` globals)** is part of `npm run lint`; a value assigned and never read, or a name never defined, fails the build.
- **`language/fr.mo` is committed and must be recompiled with every `fr.po` edit** — `npm run build:mo`. Omeka loads only the `.mo`, and the release zip is a `git archive`, so the compiled catalogue has to be in the tree. Editing `fr.po` alone changes nothing the runtime can see: before v1.49.0 three msgids were reworded and the `.mo` kept French keyed to the old spellings, so all three fell through to English with no error. `npm run lint:i18n-mo` now fails on that. There is no `msgfmt` here — `scripts/gettext.js` is the repo's own po/mo codec.
- **The `iwac-i18n.js` dictionary is checked for parity, not just duplicates.** `npm run lint:i18n` fails when a snake_case key exists in `fr` but not `en` (English visitors would read the raw `desc_…` key, because `t()` falls back to the key) or when a key exists in `en` but not `fr` (`t()` tries `en` before the key, so French visitors silently get English). A key that is *already* an English string — `'Dashboard'`, `'Loading dashboard'` — deliberately needs no `en` entry; the fallback is the translation.
...
7. Add the partial-driven asset declaration (don't enqueue manually) — the template calls `common/iwac-block-shell` with an `assets` array naming the needs and the bundle.
8. Add the block's files to `asset/js/bundles.json` (`blocks.<name>`, orchestrator last). Bump `config/module.ini` version (+ `package.json`); run `npm run build` — it runs `lint:js`, `lint:theme` (theme tokens, including `<style>` blocks in `view/`) and `lint:blocks` (registry ↔ config ↔ classes ↔ templates ↔ manifest agree) before bundling. Commit both sources and `asset/js/dist/`.
