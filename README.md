...
| References Overview | page block | **Live** — 15 panels, incl. full-text coverage + per-model LDA topics | Precompute (`generate_references_overview.py`) |
...
- **Vocabulary-onset bursts discarded.** 325 of 341 tested subjects "burst", because a subject entering the controlled vocabulary partway through the corpus reads as one long burst from its first appearance to the present. That shape — starts at first occurrence, ends at the corpus edge — is now rejected, while spikes that return to baseline, short two-year events and late surges all survive. Raising Kleinberg's gamma, the obvious lever, was measured and does not work: it is a one-off entry cost that a multi-year burst's savings dwarf.
...
**Vocabulary-onset artefacts are discarded** (v1.24.1). The first real run found a burst in 325 of 341 tested subjects — a detector firing on 95% of what it examines is not identifying the exceptional. The cause is structural: subjects enter the controlled vocabulary partway through the corpus, so one introduced in 2010 and used steadily since carries decades of structural zeroes that the automaton reads as a single burst running from its first appearance to the present. That is the subject's arrival in the index, not a change in coverage. The signature is exact — the burst starts at the subject's first occurrence *and* ends at the corpus's last year — and rejecting it spares every other shape: a subject tagged only in 2003–04, a spike that returns to baseline, and a late surge still rising at the corpus edge all survive. An earlier attempt restricted the automaton to each subject's own active span; that also removed the artefact but discarded the sharpest signals in the corpus, since a subject tagged only in 2003–04 has a two-year span that is flat within itself and bursts nowhere.
