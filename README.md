...
**On the burst rate.** 325 of 341 tested subjects burst. That looks alarming and mostly is not. Simulating pure Poisson subjects — constant underlying rate, no real burst anywhere — over this corpus's year-volume profile, about **15% burst spuriously at `s=2.0`** (6% at `s=3.0`, 2% at `s=4.0`). Noise accounts for roughly a sixth of the observed rate; the rest appears to be real, because subject coverage in a historical press archive genuinely does concentrate around events, so "at some point in 65 years this subject ran at twice its own average for a stretch" is true of most subjects. The panel ranks by burst weight and keeps the strongest 40, so a reader sees the top of that distribution regardless. `--burst-s` is the knob if "burst" should mean *exceptional* rather than *above average*; the figures above are the basis for picking a value.
