...
`--burst-s` now defaults to 3.0: a burst means a subject ran at three times its own long-run rate rather than twice. Simulation of pure Poisson subjects over this corpus's year-volume profile puts spurious bursts at ~15% at `s=2.0` and ~6% at `s=3.0`, and at 2.0 nearly every subject (325/341) qualified, which made the signal say little. Verified before the change that a sharp event, a two-year event and a late surge all still fire at 3.0, while a modest 1.5x rise and a flat series still do not.
...
Checked against the shapes that matter before changing it: a sharp event returning to baseline, a two-year event, and a late surge still rising all still fire at 3.0 (with larger weights, since the burst state sits further from the base rate); a modest 1.5× rise and a flat series still produce nothing. Raising `s` therefore shifts the panel's top-40 *ranking* as well as its membership, rather than simply truncating it.
