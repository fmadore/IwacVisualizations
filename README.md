...
- **Boundary assets ported** — normalized region/prefecture GeoJSON for Bénin, Burkina Faso and Togo, plus Côte d'Ivoire regions, lives under `asset/data/admin-boundaries/` and lazy-loads only when the administrative mode is selected. The spatial sidecar carries compact counts, bounds and paths in `country_focus`.
