...
- **Single-panel embeds** — `…/s/<site-slug>/iwac-embed/<block>/<panel>` renders one panel of a multi-panel block full-width, without the rest of the block. IWAC has no central chart registry (each of the 12 blocks has its own orchestrator), so panels are addressed by position — `panel-<n>` — enumerated identically on the gallery side and the embed side by `asset/js/charts/shared/embed.js`, which prunes the block to the requested panel after it renders.
