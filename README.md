...
- **Article metrics** — compact stat cards: word count, Flesch readability, lexical richness (MATTR), page count, language, LDA topic label (cards with missing values are silently elided). The topic card is marked as machine-generated — tinted block, mono value, tooltip — per the module's rule that model output must be distinguishable from archival metadata.
