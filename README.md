...
| Laïcité | page block | **Live** — six views: overview with the tag-vs-text Venn and per-corpus rights split, annotated timeline (year axis or Gregorian-vs-lunar seasonality), primary-source dossier, rights-gated KWIC concordance, log-likelihood collocates sliced by decade/country/corpus, and a token-normalised corpus comparison with per-outlet frame fingerprints. Scans four subsets (articles, publications, documents, references); counts are never summed across them | Precompute (`generate_laicite.py` + committed `laicite-events.json`) |
